...
def refresh_geojson_from_xy(net) -> None:
    """
    Convert net.bus x/y and line geodata into net.bus.geo / net.line.geo
    for pandapower 3.x simple_plot().
    """
    try:
        pp.plotting.geo.convert_geodata_to_geojson(net)
    except AttributeError:
        pplot.geo.convert_geodata_to_geojson(net)

def _parse_bus_geo(value):
    """
    Parse pandapower bus.geo Point geometry.

    CIGRE MV in pandapower 3.4 can store bus coordinates as GeoJSON strings
    in net.bus["geo"], not in net.bus_geodata and not in net.bus["x"]/["y"].
    """
    if pd.isna(value):
        return None
...
def get_bus_xy(net) -> pd.DataFrame:
    """
    Return bus coordinates indexed by pandapower bus index.

    Priority:
    1. net.bus_geodata["x", "y"]
    2. net.bus["x", "y"]
    3. net.bus["geo"] GeoJSON Point strings
    """
    if hasattr(net, "bus_geodata") and isinstance(net.bus_geodata, pd.DataFrame):
        gd = net.bus_geodata
        if {"x", "y"}.issubset(gd.columns):
            idx = gd.index.intersection(net.bus.index)
            out = gd.loc[idx, ["x", "y"]].copy()
    ...
    if {"x", "y"}.issubset(net.bus.columns):
        out = net.bus[["x", "y"]].copy()
        out["x"] = pd.to_numeric(out["x"], errors="coerce")
        out["y"] = pd.to_numeric(out["y"], errors="coerce")
        out = out.dropna(subset=["x", "y"])
        if len(out) >= 1:
    ...
    if "geo" in net.bus.columns:
        rows = []
        for bus_idx, value in net.bus["geo"].items():
            xy = _parse_bus_geo(value)
            if xy is not None:
                rows.append((bus_idx, xy[0], xy[1]))

        if rows:
            out = pd.DataFrame(rows, columns=["bus", "x", "y"]).set_index("bus")
            return out
...
def needs_generic_coords(net) -> bool:
    ...
    return len(get_bus_xy(net)) < 2

def auto_bus_size(net) -> float:
    """
    Returns a bus marker size scaled to the network bounding box.

    Uses get_bus_xy(), so it supports:
    - net.bus_geodata
    - net.bus x/y
    - net.bus geo
    """
    ...
    return float(np.clip(extent * 0.012, 0.01, 2.0))

def annotate_bus_ids(ax, net, fontsize: int = 5, color: str = "#cc0000") -> None:
    """Draw pandapower bus indices at their plotted coordinates."""
    gd = get_bus_xy(net)
    if gd.empty:
        return

    for bus_idx, row in gd.iterrows():
        ax.annotate(
            str(bus_idx),
...
def apply_tiny_axis_padding(ax, net, tiny_span: float = 50.0) -> None:
    """Add padding for compact generic-coordinate plots."""
    gd = get_bus_xy(net)
    if len(gd) < 2:
        return

    x_min, x_max = gd["x"].min(), gd["x"].max()
    y_min, y_max = gd["y"].min(), gd["y"].max()

    x_span = x_max - x_min
    y_span = y_max - y_min
    span = max(x_span, y_span)

    if not (0 < span < tiny_span):
        return
...
def annotate_extreme_days(ax, series: pd.Series, extreme_days: dict,
    ...
    for key, label, va in day_keys:
        ...
        ax.text(day_ts, day_val, f"  {label}", fontsize=7, color=color,
                va=va, rotation=90, clip_on=False)
...
def plot_topology(
    ...
    if is_sb:
        # SimBench: right panel is a generic schematic layout.
        # Work on a copy so the original geographic layout remains untouched.
        net_struct = copy.deepcopy(net)
        pplot.create_generic_coordinates(net_struct, overwrite=True)
        refresh_geojson_from_xy(net_struct)

        struct_bus_sz = min(auto_bus_size(net_struct), 0.08)
        ...
        # Non-SimBench: keep real geodata if it exists.
        # Only create generic coordinates when the network has no usable coordinates.
        net_struct = copy.deepcopy(net)
    ...
    if show_bus_ids:
        annotate_bus_ids(ax_right, net_struct, fontsize=6)

    ax_right.set_aspect("equal", adjustable="box")
    apply_tiny_axis_padding(ax_right, net_struct)
