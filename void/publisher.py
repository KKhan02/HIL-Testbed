...
def _dump(obj: Any, path: Path, indent: int = 2) -> None:
    ...
    with open(path, "w", encoding="utf-8") as fh:
        json.dump(_nan_to_none(obj), fh, cls=_Encoder, allow_nan=False, indent=indent)
...
class PublishHandle:
    ...
    def on_scenario_start(
        ...
        if self.enable_checkpointing:
            ...
            # NOT truncated — this is the resume source.
        else:
    ...
    def get_resume_records(self, scenario_id: str) -> list:
        ...
        Returns [] if checkpointing is disabled, no checkpoint file exists,
        or it has zero valid lines (i.e. this is a fresh run).
        """
    ...
    def on_timestep(self, rec) -> None:
        """
        Call once per timestep (from inside the scenario runner timestep loop).

        Emits a compact live frame to the JSONL file every update_every_k steps.
        The scenario runner loop checks this at negligible cost — the JSON
        serialisation only runs on the k-th step.
        """
        ...
        if self._checkpoint_path is not None:
            _append_jsonl(rec.to_checkpoint_dict(), self._checkpoint_path)
    ...
    def on_scenario_end(self, result) -> None:
        ...
        Appends a terminal "scenario_complete" event to the JSONL so the
        Streamlit app knows the stream has ended and can switch to the
...
def publish_result(
    ...
    """
    Serialise a completed BenchmarkResult to disk.

    This is the primary entry point for post-run (static) publishing.
    No callback wiring needed — call it after run_benchmark() returns.

    Parameters
    ----------
    result     : BenchmarkResult from run_benchmark().
    net        : the original pandapower network (not a scenario copy).
    profiles   : the profiles dict from build_annual_profiles().
    output_dir : root directory for all published files.
    network_id : human-readable network identifier string.

    Returns
    -------
    dict mapping logical name → Path of each written file.
        "topology"  → topology.json
        "profiles"  → profiles.json
        "hc"        → hc.json          (if HC results present)
        "scenario/<id>" → scenarios/<id>.json  (one per scenario)
    """
    out = Path(output_dir)
    ...
    # 4. Per-scenario timeseries
    (out / "scenarios").mkdir(parents=True, exist_ok=True)
    for n, scenario_result in result.results.items():
        if scenario_result is None:
            continue
        sid  = scenario_result.scenario_id
        path = out / "scenarios" / f"{sid}.json"
        _dump(build_scenario_payload(scenario_result), path)
        written[f"scenario/{sid}"] = path
    ...
    logger.info(
        "publisher: wrote %d files to %s",
        len(written), out,
    )
    for label, path in written.items():
        logger.debug("  %-30s → %s", label, path)
