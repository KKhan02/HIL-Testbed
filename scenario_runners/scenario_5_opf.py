...
def run_scenario_5(
    ...
    if publish_fn is not None:
        publish_fn.on_scenario_start("opf", "OPF (theoretical bound)", len(ap.times))

    resumed_records: list[TimestepRecord] = []
    if publish_fn is not None and enable_checkpointing:
        resumed_records = publish_fn.get_resume_records("opf")
    ...
    if start_t >= len(ap.times) and resumed_records:
        logger.info(
            "[Scenario 5 | %s] Checkpoint already covers all %d steps — skipping simulation.",
        ...
        result = ScenarioResult.from_records(
            scenario_id="opf", network_id=network_id,
        ...
        return result
    if start_t > 0:
        logger.info("[Scenario 5 | %s] Resuming from t=%d/%d.", network_id, start_t, len(ap.times))
