...
def load_dwd_solar(data_dir: str,
    ...
    Unit conversion (DWD only):
        GHI [W/m2] = Wert [J/cm2] x 10,000 / interval_seconds
        (Irradiance = (Energy/Area)/Time)
    interval_seconds is inferred from the file's own timestamp spacing
    (median gap between consecutive rows), not assumed to be 600s (10 min)
    — so this is correct for any DWD-style energy-accumulation file,
    regardless of its native reporting cadence.
    ...
    # interval_seconds: inferred from the actual data, falls back to 600s
    # (DWD's native 10-min cadence) if fewer than 2 rows are present.
    if file_map is None or "RAD-G" not in file_map:
        interval_seconds = df.index.to_series().diff().median().total_seconds() if len(df.index) > 1 else 600.0
        df["value"] = df["value"] * 10_000 / interval_seconds
...
def compute_load_profiles_bdew(net,
    ...
    for profile_type in set(load_bdew_type.values()):
        ...
        if not isinstance(raw,pd.Series):
            ...
            raw = raw.reindex(raw.index.union(times)).interpolate(method="time").reindex(times)
...
def find_extreme_days(profiles:dict) -> dict:
    ...
    def peak_day(series: pd.Series, find_max: bool):
        ...
        expected    = float(daily_count.max()) * 0.8
        daily_mean  = daily_mean[daily_count >= expected]
        if daily_mean.empty:
            return None
        ts = daily_mean.idxmax() if find_max else daily_mean.idxmin()
...
def build_annual_profiles(
    ...
    if net_type == "simbench":
        ...
        pv_mask = net.sgen["type"].str.lower().str.contains("pv|solar|lv_res", na=False)
        wind_mask = net.sgen["type"].str.lower().str.contains("wind|wp", na=False)
    ...
    # Trim to a single complete calendar year.
    # With DWD files spanning multiple years (e.g. Jan 2024 – Mar 2026),
    # passing a multi-year times index to oemof.demand BDEW causes it to
    # generate profiles only for the first year, leaving NaN/zero values
    # for subsequent years — producing the triangular spike artefacts seen
    # in day zoom plots. Selecting one complete year also makes the annual
    # profile x-axis clean (Jan–Dec rather than Dec 2024–Apr 2026).
    # Strategy: pick the most recent year that has data for all 12 months.
    years_present = sorted(set(times.year))
    target_year = None
    for yr in years_present:
        if set(times[times.year == yr].month) == set(range(1, 13)):
            target_year = yr
            break
    if target_year is None:
        # No fully complete year — fall back to the year with most timesteps
        target_year = max(years_present, key=lambda yr: (times.year == yr).sum())
        print(f"[profile_builder] WARNING: No complete calendar year found in DWD data. "
            f"Using {target_year} ({(times.year == target_year).sum()} timesteps).")
    else:
        print(f"[profile_builder] Trimming to calendar year {target_year} "
            f"(most recent complete year in DWD data).")
    times = times[times.year == target_year]
