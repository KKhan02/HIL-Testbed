 ...
 * ---------------------------------------------
 * The constants below are BOOT DEFAULTS only. At session start the RPi
 * pushes the live Q(V) parameter VALUES via the CFG: handshake (see CFG
 * semantics below), so these values need not match volt_var_controller.py.
 * Only the curve SHAPE -- piecewise-linear with a deadband -- must match
 * compute_setpoint() on the Python side. *
 ...
 *
 * CFG semantics:
 *   The five values overwrite cfg_q_ratio, cfg_u1..cfg_u4 (the Q(V)
 *   characteristic). Compile-time constants Q_RATIO / U1_PU..U4_PU now act
 *   only as boot defaults. P: is rejected (ERR:CFG_BEFORE_P) until a valid
 *   CFG has been received, making the RPi the single source of truth for
 *   the characteristic and eliminating the "constants must match in two
 *   files" invariant for the parameter VALUES (the curve SHAPE — piecewise
 *   linear with deadband — must still match compute_setpoint()).
 *
 * Per timestep:
 *   RPi  -> Arduino :  "V:<v1>,<v2>,...\n"   vm_pu per DER
 *   Arduino -> RPi  :  "Q:<q1>,<q2>,...\n"   q_mvar per DER
 ...
 *   "ERR:CFG_COUNT"       CFG did not contain exactly 5 parseable floats
 *   "ERR:CFG_INVALID"     CFG values non-finite, q_ratio out of (0, 1],
 *                         or breakpoints not strictly increasing u1<u2<u3<u4
 *   "ERR:P_BEFORE_INIT"   P message received before valid INIT
 *   "ERR:CFG_BEFORE_P"    P message received before valid CFG
 ...
 *   cfg_q_ratio, cfg_u1,
 *   cfg_u2, cfg_u3, cfg_u4   : 5 x 4  =  20 bytes  (CFG-configurable Q(V))
 *   n_ders, configured,
 *   configured_cfg,
 *   skip_to_newline, buf_idx : ~7 bytes
 *   Subtotal declared        : ~1737 bytes
 *
 ...
 *
 *   Total (estimated)          : 1939 bytes  (was 1918 pre-CFG; +21 bytes)
 *   Remaining for stack        :  109 bytes  (2048 - 1939)
 *   Estimated stack usage      :  ~80 bytes  (handle_message + loop frames;
 *                                 the CFG handler adds NO stack — it reuses
 *                                 vm_buf[] as parse scratch, no local arrays)
 *   Free margin                :  ~29 bytes  <- verify with compiler after
 *                                 flashing; if margin proves insufficient,
 *                                 apply Option A below (MAX_DERS reduction)
 *                                 or trim BUF_SIZE 870 -> 850 (still > 842
 *                                 typical worst case) to reclaim 20 bytes.
 *
...
// ===========================================================================
// Q(U) characteristic BOOT DEFAULTS
// ---------------------------------------------------------------------------
// Since the CFG protocol step, these are defaults only. The operative values
// live in cfg_q_ratio / cfg_u1..cfg_u4 (see Global state) and are overwritten
// by the CFG: message from the RPi at session startup. The RPi module
// volt_var_controller.py (Q_RATIO, U1_PU..U4_PU) is the single source of
// truth. P: is rejected until CFG has been received, so these defaults are
// never used for an actual computation in a correctly-sequenced session.
// ===========================================================================
...
static float    vm_buf[MAX_DERS];     /* voltages parsed from V: — global to avoid 420-byte stack frame.
                                         Also reused as parse scratch by the CFG: handler (first 6 slots)
                                         — safe because CFG precedes P/V and vm_buf is dead until V:. */
static int      n_ders         = 0;     /* number of DERs (set by INIT) */
static bool     configured     = false; /* true after successful P handshake */
static bool     configured_cfg = false; /* true after successful CFG handshake */
static bool     skip_to_newline = false; /* set on BUF_OVERFLOW; discard tail bytes */

/* Q(V) characteristic — runtime-configurable via CFG:. Initialised to the
 * compile-time defaults so compute_q() is well-defined even before CFG
 * (e.g. bench testing over a serial monitor with configured_cfg guards
 * bypassed). 5 floats = 20 bytes SRAM; accounted in the header budget. */
static float    cfg_q_ratio = Q_RATIO;
static float    cfg_u1      = U1_PU;
static float    cfg_u2      = U2_PU;
static float    cfg_u3      = U3_PU;
static float    cfg_u4      = U4_PU;
...
float compute_q(float vm_pu, float p_inst_mw) {
    /* Uses the CFG-supplied runtime parameters, not the compile-time
     * defaults. ERR:CFG_INVALID guarantees cfg_u1 < cfg_u2 < cfg_u3 < cfg_u4
     * at commit time, so both ramp denominators are strictly positive. */
    float q_max = cfg_q_ratio * fabsf(p_inst_mw);

    if (vm_pu <= cfg_u1) {
        return q_max;
    } else if (vm_pu < cfg_u2) {
        return q_max * (cfg_u2 - vm_pu) / (cfg_u2 - cfg_u1);
    } else if (vm_pu <= cfg_u3) {
        return 0.0f;
    } else if (vm_pu < cfg_u4) {
        return -q_max * (vm_pu - cfg_u3) / (cfg_u4 - cfg_u3);
    } else {
        return -q_max;
    }
}
...
void handle_message(char *msg) {
    ...
    /* ---- CFG:<q_ratio>,<u1>,<u2>,<u3>,<u4> ---- */
    if (strncmp(msg, "CFG:", 4) == 0) {
        /*
         * Parse scratch: reuse vm_buf[] (dead until the first V: message,
         * which the protocol guarantees comes after CFG). Zero additional
         * stack or SRAM cost. max_n = 6 (not 5) so that a sixth value is
         * detected as "too many" instead of being silently truncated.
         *
         * Two-phase parse-then-commit: cfg_* globals are only written after
         * count and validity checks pass, so a malformed CFG can never leave
         * the characteristic in a half-updated, inconsistent state.
         */
        int n = parse_float_array(msg + 4, vm_buf, 6);
        if (n != 5) {
            Serial.println(F("ERR:CFG_COUNT"));
            return;
        }

        /* Validity: finite values, q_ratio in (0, 1], strictly increasing
         * breakpoints. Guarantees positive ramp denominators in compute_q()
         * and physically meaningful Q_max. */
        for (int i = 0; i < 5; i++) {
            if (isnan(vm_buf[i]) || isinf(vm_buf[i])) {
                Serial.println(F("ERR:CFG_INVALID"));
                return;
            }
        }
        if (vm_buf[0] <= 0.0f || vm_buf[0] > 1.0f ||
            !(vm_buf[1] < vm_buf[2] && vm_buf[2] < vm_buf[3] && vm_buf[3] < vm_buf[4])) {
            Serial.println(F("ERR:CFG_INVALID"));
            return;
        }

        cfg_q_ratio    = vm_buf[0];
        cfg_u1         = vm_buf[1];
        cfg_u2         = vm_buf[2];
        cfg_u3         = vm_buf[3];
        cfg_u4         = vm_buf[4];
    ...
    if (strncmp(msg, "P:", 2) == 0) {
        ...
        if (!configured_cfg) {
            /* Protocol ordering guard: INIT -> CFG -> P. Old RPi software
             * that skips CFG fails here with an explicit, diagnosable error
             * rather than silently running compile-time default parameters. */
            Serial.println(F("ERR:CFG_BEFORE_P"));
            return;
        }
