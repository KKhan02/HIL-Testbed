...
UNBALANCE_MAX_PERCENT = 2.0    # %    IEC 62749 voltage unbalance limit
THERMAL_LOADING_PLAUSIBLE_MAX = 300.0

# Framework defaults captured at import, so a long-lived process (notebook,
# server) can restore a pristine baseline between runs. set_limit() mutates
# module globals process-wide and has no implicit reset; without this a
# limit overridden by run A leaks into run B when run B leaves it default.
...
def reset_limits() -> None:
    """Restore all limit constants to their import-time framework defaults.
    Call before applying a new run's overrides in a shared interpreter."""
    for attr, val in _LIMIT_DEFAULTS.items():
        globals()[attr] = val
...
def set_limit(attr: str, value, lo: float, hi: float, unit: str, warn=print, confirm=None) -> None:
    """
    Validated setter for this module's limit constants (V_MIN, V_MAX,
    LINE_MAX_LOADING, TRAFO_MAX_LOADING, VA_DIFF_MAX_DEGREE,
    UNBALANCE_MAX_PERCENT). Shared by executor.py's apply_violation_limits()
    (CLI wizard path) and run_benchmark_script.py (direct-script path) so
    both entry points apply the same range checks and fallback-on-invalid
    behaviour instead of maintaining separate copies.

    None is a no-op (leaves the current value unchanged). An invalid
    value keeps the current value and calls warn() with a message
    instead of raising. If confirm is given, it is called with a message
    whenever the value actually changes; left as None (no-op) by default
    so callers that don't want a confirmation line (e.g. the direct-script
    path) get silence on success.
    """
    import numpy as np
    current = globals()[attr]
    if value is None:
        return
    try:
        v = float(value)
        ...
        ok = False
    if not ok:
        warn(f"Ignoring invalid {attr}={value!r} (expected "
             f"{lo} < value <= {hi} {unit}); keeping {current}.")
        return
    if v != current:
            globals()[attr] = v
            if confirm is not None:
                confirm(f"{attr}: {current} -> {v} {unit} (run-wide)")
...
def detect_violations(
        ...
        v_max:       float = None,
        line_max:    float = None,
        trafo_max:   float = None,
        va_diff_max: float = None,
) -> ViolationReport:
    """
    Detect all planning constraint violations after runpp() has been called.

    Threshold defaults are LATE-BOUND: a None argument resolves to the
    module-level constant (V_MIN, V_MAX, LINE_MAX_LOADING, TRAFO_MAX_LOADING,
    VA_DIFF_MAX_DEGREE) at CALL time, not at import time. This lets the CLI
    executor apply user-configured limits process-wide by rebinding the
    module constants before a run, without threading five extra kwargs
    through every runner. Explicit arguments always win over the constants.
    ...
    Hosting capacity analysis::
        ...
        while True:
            ...
            pv_scale += step
    """
    # Late-bound defaults — resolve against the CURRENT module constants so
    # runtime overrides (CLI executor) apply everywhere defaults are used.
    if v_min       is None: v_min       = V_MIN
    if v_max       is None: v_max       = V_MAX
    if line_max    is None: line_max    = LINE_MAX_LOADING
    if trafo_max   is None: trafo_max   = TRAFO_MAX_LOADING
    if va_diff_max is None: va_diff_max = VA_DIFF_MAX_DEGREE
...
def detect_violations_3ph(
    ...
    Example — custom parameterised LV network::
        ...
        report = detect_violations_3ph(net)
    """
    # Late-bound defaults — same mechanism as detect_violations().
    if v_min         is None: v_min         = V_MIN
    if v_max         is None: v_max         = V_MAX
    if line_max      is None: line_max      = LINE_MAX_LOADING
    if trafo_max     is None: trafo_max     = TRAFO_MAX_LOADING
    if unbalance_max is None: unbalance_max = UNBALANCE_MAX_PERCENT
